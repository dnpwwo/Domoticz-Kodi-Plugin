...
class Packet:
    ...
    def append_payload(self, blob):
        ...
        if (isinstance(blob, str)):
            self.set_payload(self.payload + bytearray(blob, "utf-8"))
        else:
            self.set_payload(self.payload + bytearray(blob))
    ...
    def set_payload(self, payload):
        ...
        if (isinstance(payload, str)):
            self.payload = bytearray(payload, "utf-8")
        else:
    ...
    def get_header(self, packettype=-1, seq=1, maxseq=1, payload_size=0):
        ...
        header = bytes(self.sig)
        header += bytes(self.majver)
        header += bytes(self.minver)
        header += bytes(format_uint16(packettype))
        header += bytes(format_uint32(seq))
        header += bytes(format_uint32(maxseq))
        header += bytes(format_uint16(payload_size))
        header += bytes(format_uint32(self.uid))
        header += bytes(self.reserved)
        return header
...
class PacketNOTIFICATION (Packet):
    ...
    def __init__(self, title, message, icon_type=ICON_NONE, icon_file=None):
        ...
        self.set_payload ( format_string(title).encode('utf-8') )
        self.append_payload( format_string(message).encode('utf-8') )
        self.append_payload( chr(icon_type).encode('utf-8') )
        self.append_payload( format_uint32(0) ) # reserved
        if icon_type != ICON_NONE and icon_file:
            fIcon = open(icon_file, "rb")
            self.append_payload( fIcon.read() )
